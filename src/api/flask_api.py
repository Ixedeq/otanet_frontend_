...
def generate_proxied_image_url(image_url):
    ...
    try:
        ...
        if len(path_parts) >= 3:
            hash_value = path_parts[-2]
            page_filename = path_parts[-1]
            return f"/api/image/{hash_value}/{page_filename}"
    except Exception:
        pass
...
@app.route('/api/image/<hash_id>/<page_filename>', methods=['GET'])
def proxy_fetch(hash_value, page_filename):
    """
    Proxy image requests from MangaDex to bypass CORS restrictions
    """
    try:
        image_url = f"https://uploads.mangadex.org/data/{hash_value}/{page_filename}"
